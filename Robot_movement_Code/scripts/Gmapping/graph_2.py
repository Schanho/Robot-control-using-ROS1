...
ax_ate.plot(labels, ates, marker='o', color='blue', linestyle='-', label='ATE')
ax_ate.plot(labels, [0] * len(labels), color='black', linestyle='--', label='Ground Truth')
...
# ATE 그래프 저장
plt.grid(True)
plt.savefig('/home/dfx/catkin_ws/src/my_robot/scripts/Gmapping/map/ate_comparison_line.png')
plt.close()

# RMSE 비교 꺾은선 그래프
fig, ax_rmse = plt.subplots(figsize=(10, 6))
ax_rmse.plot(labels, rmses, marker='o', color='red', linestyle='-', label='RMSE')
ax_rmse.plot(labels, [0] * len(labels), color='black', linestyle='--', label='Ground Truth')
...
# RMSE 그래프 저장
plt.grid(True)
plt.savefig('/home/dfx/catkin_ws/src/my_robot/scripts/Gmapping/map/rmse_comparison_line.png')
plt.close()