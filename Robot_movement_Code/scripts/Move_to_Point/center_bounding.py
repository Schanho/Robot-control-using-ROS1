...
def main():
    ...
    # 카메라 캘리브레이션 데이터 (미리 캘리브레이션된 값을 사용)
    camera_matrix = np.array([[618.907, 0, 320.579], [0, 618.536, 240.748], [0, 0, 1]])  # 예시 값
    dist_coeffs = np.array([0.14457, -0.25288, -0.00151, 0.00074, 0.00000])  # 예시 값
    ...
    try:
        while not rospy.is_shutdown():
            ...
            # 카메라 왜곡 보정
            rgb_frame = cv2.undistort(rgb_frame, camera_matrix, dist_coeffs)
            ...
            if ids is not None:
                ...
                for i, marker_id in enumerate(ids):
                    if marker_id == 1:  # ID 1 마커가 인식되면
                        ...
                        if marker_center_x < frame_center_x - 30:  # 민감도 조정
                            rospy.loginfo("Marker is to the left. Rotating right.")
                            move_robot(0, 0.15, 0.1)  # 좌회전
                        elif marker_center_x > frame_center_x + 30:  # 민감도 조정
                            rospy.loginfo("Marker is to the right. Rotating left.")
                            move_robot(0, -0.15, 0.1)  # 우회전
                        else:
                            # 마커가 중앙에 있으면 앞으로 전진
